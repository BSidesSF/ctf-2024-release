...
enum s20_status_t s20_crypt(uint8_t *key, enum s20_keylen_t keylen, uint8_t nonce[static 8], uint32_t si, uint8_t *buf, uint32_t buflen);
...
int main(int argc, char *argv[])
{
  ...
  // Store length in a register to avoid wrecking it when we overflow the stack
  register int length = atoi(strlength);
  read(fileno(stdin), data, length);

  s20_crypt(k, S20_KEYLEN_128, n, 0, (uint8_t*)data, length);

  if(!memcmp(data, "password=MyCoolestPassword", length)) {
