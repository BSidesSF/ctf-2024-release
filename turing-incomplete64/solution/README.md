This is a write-up for `turing-complete` and `turing-incomplete` from the BSides San Francisco 2024 CTF!

`turing-complete` is a 101-level reversing challenge, and `turing-incomplete` is a much more difficult exploitation challenge with a very similar structure.
